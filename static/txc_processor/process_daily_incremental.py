"""
Process daily TransXChange updates - INCREMENTAL UPDATE
Only updates patterns that have changed, preserves rest
"""
...
def parse_journey_pattern_section(section_elem):
    """Extract stops from JourneyPatternSection"""
    stops = []
    cumulative_time = 0
...
print("=" * 80)
print("DAILY SCHEDULE UPDATE - INCREMENTAL")
print("=" * 80)
...
# Load existing patterns to identify which ones are affected
...
print(f"✓ Loaded {len(db_patterns):,} existing patterns")

# Process daily XMLs
print("\nProcessing daily TransXChange files...")
xml_files = list(EXTRACT_DIR.rglob("*.xml"))
print(f"Found {len(xml_files):,} XML files")

# Track which patterns are affected and their validity periods
...
pattern_validity = {}  # Track validity dates per pattern
...
for xml_file in xml_files:
    try:
        ...
        for vj in root.findall('.//txc:VehicleJourney', NS):
            ...
            # Track this pattern as affected
            affected_pattern_ids.add(pattern_id)
            
            # Store validity dates for this pattern
            if pattern_id not in pattern_validity:
                pattern_validity[pattern_id] = (valid_from, valid_to)
...
print(f"  Affected patterns: {len(affected_pattern_ids)}")

# CRITICAL: Delete ONLY affected patterns
if len(affected_pattern_ids) > 0:
    print(f"\nDeleting old schedules for {len(affected_pattern_ids)} affected patterns...")
    
    pattern_ids_list = list(affected_pattern_ids)
    cur.execute(
        "DELETE FROM stop_schedule WHERE pattern_id = ANY(%s)",
        (pattern_ids_list,)
    )
...
# Build new schedule records for affected patterns only
print("\nBuilding new schedule records...")
db_records = []
...
for naptan_id, patterns in stop_schedules.items():
    for pattern_id, validity_periods in patterns.items():
        for validity_key, days in validity_periods.items():
            # Extract dates from validity key
            ...
            for day, hours in days.items():
                for hour, times in hours.items():
                    ...
                    db_records.append((
                        ...
                        vf, vt  # Use actual validity dates from XML
                    ))

print(f"✓ Built {len(db_records):,} new records")

# Insert new records
if len(db_records) > 0:
    print("\nInserting updated schedules...")
    batch_size = 10000
    ...
    conn.commit()
    print(f"✓ Inserted {len(db_records):,} new records")

# Summary
cur.execute("SELECT COUNT(*) FROM stop_schedule")
total_records = cur.fetchone()[0]

cur.execute("SELECT COUNT(DISTINCT pattern_id) FROM stop_schedule")
...
print("=" * 80)
print(f"Affected patterns: {len(affected_pattern_ids)}")
print(f"Deleted old records: {deleted_count:,}")
print(f"Inserted new records: {len(db_records):,}")
